  ...
  const handleCreateClick = async () => {
    try {
      const newPoll = {
        ...
        isActive: true
        // options: options.map((option) => option.title).filter(Boolean)
      };

      const response = await PollService.createPoll(newPoll);
      console.log('Poll created:', response); // Handle the response as needed
    } catch (error) {
      ...
      <input
        ...
        on:input={(e) => {
          ...
          if (target instanceof HTMLInputElement) {
            updateOption(option.id, target.value);
          }
        }}
      />
      <button
